...
def Get_Data_Label_Aux_train_Set(data_train, steps):
    cabinets = data_train
    stamps = data_train
    x_dim = len(cabinets)
    time_dim = len(stamps)
...
def Get_Data_Label_Aux_test_Set(data_test, steps):
    cabinets = data_test
    stamps = data_test
    x_dim = len(cabinets)
    time_dim = len(stamps)
...
if __name__ == "__main__":
    ...
    X_full, Y_full, hour_full, dayofweek_full = Get_Data_Label_Aux_train_Set(data_train, time_lag)

    print('X_full train shape: ', X_full.shape, 'Y_full shape:', Y_full.shape)
    X_train, Y_train  = Split_train_Data(X_full, Y_full)

    X_test_full, Y_test_full, hour_full, dayofweek_full = Get_Data_Label_Aux_test_Set(data_test, t_time_lag)
    X_test, Y_test = Split_test_Data(X_test_full, Y_test_full)
    print('X_full test shape: ', X_test.shape, 'Y_full shape:', Y_test.shape)
   ...
   # model = load_model('H:\\Paper Code\\timelag_3__LSTMS_GRU-___-mse-rmse85ep_tl3.h5', custom_objects={'Attention': Attention})
    model = load_model('H:\\Paper Code\\timelag_3__LSTMS_GRU-___-mse-rmse85ep_tl3.h5')
    model.summary()
