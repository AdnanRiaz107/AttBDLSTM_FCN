...
if __name__ == "__main__":
  ...
  #  X_min = np.min([np.min(X_train), np.min(X_test)])
   ...
   # Y_test_scale = Y_test / X_max
    ...
    FCN_BDLSTM_MSE_RMSE, history_2__LSTM = ML_BDLSTM_FCN.FCN(X_train_scale, Y_train_scale, epochs=model_epoch)

    FCN_BDLSTM_MSE_RMSE.save('TL12--FCN for TTL-6- ' + str(len(history_2__LSTM.losses)) + 'ep' + '_tl' + str(time_lag) + '.h5')
   ...
   # aa = [x for x in range(350)]
    ...
    plt.plot( Y_pred_test[:, 0, :], 'r', label= 'predicted')
