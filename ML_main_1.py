...
def Get_Data_Label_Aux_test_Set(data_test, steps, t_tlag):
    ...
    for i in range(steps, time_dim - steps):
        data_set.append(data_test[i - steps: i])
...
if __name__ == "__main__":
   ...
   # print('X_full max:', X_max)
    ...
    FCN_BDLSTM_MSE_RMSE, history_2__LSTM = ML_BDLSTM_FCN.BDLSTMS(X_train_scale, Y_train_scale, epochs=model_epoch)

    FCN_BDLSTM_MSE_RMSE.save('TL3-BDLSTMs81 for TTL-12---' + str(len(history_2__LSTM.losses)) + 'ep' + '_tl' + str(time_lag) + '.h5')
    ...
    plt.plot(y_true[:, 0, :], marker='.')
    plt.plot(Y_pred_test[:, 0, :], 'r')
