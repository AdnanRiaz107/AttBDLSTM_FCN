...
def SplitData2(X_full, Y_full,t_timelag, hour_full, dayofweek_full, train_prop=0.7, valid_prop=0.2, test_prop=0.1):
    n = Y_full.shape[0]
    indices = np.arange(n)
    RS = RandomState(1024)
    RS.shuffle(indices)
    ...
    #########################################
    # here need to make changes in testing data time stamp, for predicion future
    ###############################


    for i in range(6, len(test_indices),-6):
        test_indices.append(test_indices[i-6:i])


    print("test incides shape")
    print(test_indices)
...
if __name__ == "__main__":
    ...
    FCN_BDLSTM_MSE_RMSE, history_2__LSTM = BDLSTM_FCN.BDLSTMS_FCN(X_train_scale, Y_train_scale, epochs=model_epoch)

    FCN_BDLSTM_MSE_RMSE.save('TL-12__Proposed_Model' + str(len(history_2__LSTM.losses)) + 'ep' + '_tl' + str(time_lag) + '.h5')
