...
def Get_Data_Label_Aux_train_Set(data_train, steps, t_tlag):
  #  cabinets = data_train.columns.values
  #  stamps = data_train.index.values
  #  x_dim = len(cabinets)
  #  time_dim = len(stamps)

  #  data_train = data_train.iloc[:, :].values
    ...
    for i in range(time_dim - steps):
        data_set.append(data_train[i: i + steps])
      #  label_set.append(data_train[i + steps])
...
def Get_Data_Label_Aux_test_Set(data_test, steps, t_tlag):
   # cabinets = data_test.columns.values
   # stamps = data_test.index.values
   # x_dim = len(cabinets)
  #  time_dim = len(stamps)

  #  data_test = data_test.iloc[:, :].values
    cabinets = data_test
    stamps = data_test
    x_dim = len(cabinets)
    time_dim = len(stamps)
...
if __name__ == "__main__":
  ...
  #  data_train, data_test = train_test_split(speedMatrix, test_size=0.10, random_state=1024)

    time_lag = 3
    t_time_lag = 3
    print('time lag :', time_lag)
    print('time T-lag :', t_time_lag)
    ...
    X_max = np.max([np.max(X_train)])
    X_mix = np.min([np.min(X_train)])
    ...
    FCN_BDLSTM_MSE_RMSE, history_2__LSTM = ML_BDLSTM_FCN.LSTMDNN(X_train_scale, Y_train_scale, epochs=model_epoch)

    FCN_BDLSTM_MSE_RMSE.save('TL3-CheckLSTM-DNN for TTL-3- ' + str(len(history_2__LSTM.losses)) + 'ep' + '_tl' + str(time_lag) + '.h5')
  ...
  #  X_max = np.max([np.max(X_test)])
   # print('X FUll Test', X_max)
    ...
    x= concatenate()(Y_pred_test) * X_max
    y= concatenate()( y_true) * X_max
    df['act']= pd.DataFrame(x)
    df['pred']= pd.DataFrame(y)



   # aa = [x for x in range(350)]
    plt.figure(figsize=(15, 12))
    plt.plot(df['act'][:, 0, :], label='actual')
  #  plt.plot(y_true[:, 0, :], marker='.', label= 'actual')
    plt.plot( df['pred'][:, 0, :], 'r')
    plt.tick_params(left=False, labelleft=True, label= 'predicted')
    plt.tight_layout()
    ...
    Y_pred_test = []
    random_day = random.randint(0,0,6)
    idx = random_day * 100
